...
Lxs=[0]*train_iters*epoch
Lzs=[0]*train_iters*epoch
for e in range(epoch):
        ...
        for i in range(train_iters):
                ...
                Lxs[e*train_iters + i],Lzs[e*train_iters +i],_=results
        if e%10==0:
                iter_time = time.time()
                print("epoch=%d : Lx: %f Lz: %f time: %f" % (e,np.mean(Lxs[e*train_iters:(e+1)*train_iters]),np.mean(Lzs[e*train_iters:(e+1)*train_iters]), iter_time - start_time))
