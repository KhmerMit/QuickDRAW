...
batch_size=10 # training minibatch size, must be a square for output
...
train_data = np.load('data_batches2/batch_3.npy')
...
#last_save = None
#saver.restore(sess, last_save) # to restore from model, uncomment this line
...
out_file = os.path.join(FLAGS.data_dir,name_loss +".npz")
...
num_classes = 10
for k in range(num_minibatches,num_minibatches+1):
        validation_data = np.load(name_data_batches + str(k) + '.npy')
        ...
        out_file=os.path.join(FLAGS.data_dir,"draw_data_all_256" + str(k) + ".npz")
...
validation_data = np.load('data_batches2/batch_7.npy')[:batch_size] 
...
out_file = os.path.join(FLAGS.data_dir,"draw_validation_loss_" + str(k) + ".npz")
