...
#data = pd.read_csv("airplane.csv".format(draw_type))
data = pd.read_csv("cat.csv".format(draw_type))
data.head()
...
recognized = data["recognized"].values
...
#               range(len(drawings))
for draw_idx in range(2000):
    if recognized[draw_idx] == True:
        ...
        #plt.imsave(os.path.join(out_dir,'{0}.png'.format(drawing_ids[draw_idx])), resize_image, cmap='binary')
                # Labels
        #if recognized[draw_idx] == True:
         #   finding = 1
